...
namespace OrderORM
{
    public class WorklistQuerier
    {
        ...
        private DicomDataset BuildQueryDataset()
        {
            ...
            ds.Add(DicomTag.PatientName, _config.Query.PatientName ?? "");
            ...
            ds.Add(DicomTag.StudyInstanceUID, ""); // Ensure returned
            return ds;
        }
            ...
            try
            {
                ...
                if (success && _findResponses.Count > 0)
                {
                    ...
                    foreach (var dataset in _findResponses)
                    {
                        ...
                        if (HL7Sender.SendOrm(ormMessage, _config.HL7.ReceiverHost, _config.HL7.ReceiverPort))
                        {
                            CacheManager.SaveToCache(studyInstanceUid, ormMessage, _config.Cache.Folder);
                            // If this was a retry, remove from pending queue
                            if (RetryManager.IsPendingRetry(studyInstanceUid, _config.Cache.Folder))
                            {
                                RetryManager.RemovePendingMessage(studyInstanceUid, _config.Cache.Folder);
                        ...
                        else
                        {
                            ...
                            if (RetryManager.IsPendingRetry(studyInstanceUid, _config.Cache.Folder))
                            {
                                attemptCount = RetryManager.GetAttemptCount(studyInstanceUid, _config.Cache.Folder) + 1;
                            }
                            RetryManager.SavePendingMessage(studyInstanceUid, ormMessage, _config.Cache.Folder, attemptCount);
                        }
                    }
                }
        ...
        public void ProcessPendingMessages()
        {
            try
            {
                ...
                var pendingMessages = RetryManager.GetPendingMessages(_config.Cache.Folder, cutoffTime);
                ...
                foreach (var pending in pendingMessages)
                {
                    ...
                    if (HL7Sender.SendOrm(pending.OrmMessage, _config.HL7.ReceiverHost, _config.HL7.ReceiverPort))
                    {
                        // Success! Save to successful cache and remove from pending
                        CacheManager.SaveToCache(pending.StudyInstanceUid, pending.OrmMessage, _config.Cache.Folder);
                        RetryManager.RemovePendingMessage(pending.StudyInstanceUid, _config.Cache.Folder);
                    ...
                    else
                    {
                        ...
                        RetryManager.SavePendingMessage(pending.StudyInstanceUid, pending.OrmMessage, _config.Cache.Folder, pending.AttemptCount + 1);
                    }
                }
            }
            catch (Exception ex)
